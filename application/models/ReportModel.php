...
class ReportModel extends MasterModel{
    ...
    public function getLedgerBalance($data){
        ...
        $result = $this->db->query("SELECT abs(ifnull(lb.op_balance,0)) as op_balance, ifnull(lb.op_balance,0) as opb, (CASE WHEN lb.op_balance > 0 THEN 'Cr' WHEN lb.op_balance < 0 THEN 'Dr' ELSE '' END) as op_balance_type,abs(ifnull(lb.cl_balance,0)) as cl_balance, ifnull(lb.cl_balance,0) as clb, (CASE WHEN lb.cl_balance > 0 THEN 'Cr' WHEN lb.cl_balance < 0 THEN 'Dr' ELSE '' END) as cl_balance_type FROM (SELECT SUM((CASE WHEN trans_date < '".$from_date."' THEN (net_amount * p_or_m) ELSE 0 END)) as op_balance,SUM((CASE WHEN trans_date <= '".$to_date."'  THEN (net_amount * p_or_m) ELSE 0 END)) as cl_balance FROM order_transaction WHERE is_delete = 0  AND party_id = ".$data['party_id'].") as lb ")->row();
        //$this->printQuery();
        return $result;
    }
    ...
    public function getLedgerTrans($data){
        ...
        $data['select'] = "order_transaction.trans_number,DATE_FORMAT(order_transaction.trans_date,'%d-%m-%Y') as trans_date,order_transaction.net_amount, order_transaction.p_or_m, IF(order_transaction.p_or_m > 0, order_transaction.net_amount, 0) as cr_amount, IF(order_transaction.p_or_m < 0, order_transaction.net_amount, 0) as dr_amount,IF(order_transaction.net_amount > 0,order_transaction.c_or_d,'') as c_or_d,ifnull(order_transaction.remark,'') as remark,(CASE WHEN entry_type = 5 THEN 'Order' WHEN entry_type = 6 THEN 'Payment' ELSE '' END) as entry_name";
